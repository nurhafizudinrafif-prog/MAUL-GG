...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginAdmin = (passwordOrPin: string, username?: string): boolean => {
    ...
    const currentPass = (settings.adminPin || 'admin123').trim();
    const currentUser = (settings.adminUsername || 'admin').trim().toLowerCase();

    // 1. Guaranteed master default match (ALWAYS works for 'admin' and 'admin123')
    const isMasterDefault = (cleanUser === '' || cleanUser === 'admin') && cleanPass === 'admin123';

    // 2. Dynamic match from store settings
    const isCustomMatch = (cleanUser === '' || cleanUser === currentUser) && cleanPass === currentPass;

    if (isMasterDefault || isCustomMatch) {
      setIsAdminLoggedIn(true);
      localStorage.setItem('rxfif_admin_auth', 'true');
      return true;
    }
    return false;
  };
